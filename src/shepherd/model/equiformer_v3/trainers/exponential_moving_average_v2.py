"""
    1.  The code is copied from here: https://github.com/FAIR-Chem/fairchem/blob/be0f727a515582b01e6c51672a08f5b693f015e9/ocpmodels/modules/exponential_moving_average.py
    2.  We move the extra copy of parameters to CPU when evaluating with EMA weights.
"""
...
class ExponentialMovingAverageV2:
    ...
    def update(
        ...
        Args:
          parameters: Iterable of `torch.nn.Parameter`; usually the same set of
            ...
            parameters with which this `ExponentialMovingAverageV2` was
    ...
    def copy_to(
        ...
        parameters = self._get_parameters(parameters)
        for s_param, param in zip(self.shadow_params, parameters):
            model_ema_param = s_param
            param.data.copy_(model_ema_param.data)
    ...
    def store(
        ...
        Args:
          parameters: Iterable of `torch.nn.Parameter`; the parameters to be
            ...
            `ExponentialMovingAverageV2` was initialized will be used.
        """
        parameters = self._get_parameters(parameters)
        for param in parameters:
            temp = param.detach().to('cpu')
            self.collected_params.append(temp)
    ...
    def restore(
        ...
        for c_param, param in zip(self.collected_params, parameters):
            model_param = c_param.to(param.device)
            param.data.copy_(model_param.data)


    def state_dict(self) -> dict:
        r"""Returns the state of the ExponentialMovingAverageV2 as a dict."""
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        r"""Loads the ExponentialMovingAverageV2 state.

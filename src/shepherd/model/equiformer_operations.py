...
deleted SeparableFCTP from this file.
"""
...
import e3nn
from e3nn import o3
from e3nn.math import perm, normalize2mom
...
def convert_e3nn_to_equiformerv3(input_tensor, lmax, num_channels):
    out = []
    start_idx = 0
...
def convert_equiformerv3_to_e3nn(input_tensor, lmax):
    num_channels = input_tensor.shape[-1] # not used directly  # noqa: F841
    out = []
    start_idx = 0
...
def sort_irreps_even_first(irreps):
    ...
    p = perm.inverse(inv)
    irreps = o3.Irreps([(mul, (l, -p)) for l, p, _, mul in out])  # noqa: E741

"""
Edited for ShEPhERD v2.
- Added input_sphere_channels to the arguments of EquivariantGraphAttention.__init__ and TransBlockV3.__init__ as we do in ShEPhERDv1.
- Swapped the two nn.Embedding(...) for nn.Linear(input_sphere_channels, edge_channels_list[-1]) in EquivariantGraphAttention.__init__.
- Removed source_atomic_numbers/target_atomic_numbers from TransBlockV3.forward and manually extract from x l=0
"""
import torch
import math
...
class EquivariantGraphAttention(torch.nn.Module):
    """
        ...
        Adjusted to add input_sphere_channels to the arguments as we do in ShEPhERDv1.

        Args:
            input_sphere_channels (int): Number of input spherical channels (for nodes) # not in original eqV3
            ...
            [REMOVED] max_num_elements (int):     Maximum number of atomic numbers # removed since we use linear instead of embedding
    ...
    def __init__(
        ...
        # max_num_elements, # removed since we use linear instead of embedding
        edge_channels_list,
        ...
        if self.use_atom_edge_embedding:
            ...
            """
            # Edited to use linear instead of embedding
            self.source_embedding = torch.nn.Linear(self.input_sphere_channels, self.edge_channels_list[-1])
            self.target_embedding = torch.nn.Linear(self.input_sphere_channels, self.edge_channels_list[-1])
    ...
    def forward(
        ...
        if self.use_atom_edge_embedding:
            ...
            source_features = x[edge_index[0], 0, :] # l=0 embeddings only
            target_features = x[edge_index[1], 0, :] # l=0 embeddings only
            source_embedding = self.source_embedding(source_features)
            target_embedding = self.target_embedding(target_features)
...
class TransBlockV3(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ga = EquivariantGraphAttention(
            ...
            # max_num_elements=max_num_elements, # removed since we use linear instead of embedding

...
class EdgeDegreeEmbedding(torch.nn.Module):
    """
    Added input_sphere_channels to the arguments as we do in ShEPhERDv1.
        Args:
            ...
            [deleted] max_num_elements (int):         Maximum number of atomic numbers
    ...
    def forward(
        ...
        """
        if self.use_atom_edge_embedding:
            source_embedding = x_input[edge_index[0], 0, :] # l=0 embeddings only
            target_embedding = x_input[edge_index[1], 0, :] # l=0 embeddings only
        ...
        outputs = reduce_edge(
            ...
            output_shape=[x_input.shape[0], x_edge.shape[1], x_edge.shape[2]]
        )

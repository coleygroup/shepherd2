...
def _initialize_x1_state(batch_size: int,
    ...
    N_x1: The number of x1 nodes (int for uniform, list[int] of length batch_size for per-molecule).
    params: The parameters.
    prior_noise_scale: The prior noise scale.
    include_virtual_node: Whether to include a virtual node.
    scaffold_com: The scaffold center of mass.

    Returns
    -------
    Tuple: (length = 6)
        pos_forward_noised_x1: torch.Tensor (sum(N_x1_b) + batch_size, 3)
            The x1 position tensor. (first index per molecule is virtual node)
        x_forward_noised_x1: torch.Tensor (sum(N_x1_b) + batch_size, num_atom_types)
            The x1 feature tensor. (first index per molecule is virtual node)
            num_atom_types is the number of atom types + charge bins
        bond_edge_x_forward_noised_x1: torch.Tensor (total_bond_edges, len(params['dataset']['x1']['bond_types']))
            The x1 bond edge feature tensor.
        x1_batch: torch.Tensor (sum(N_x1_b) + batch_size,)
            The x1 batch tensor.
        virtual_node_mask_x1: torch.Tensor (sum(N_x1_b) + batch_size,)
            The x1 virtual node mask tensor.
        bond_edge_index_x1: torch.Tensor (2, total_bond_edges)
            The x1 bond edge index tensor.
    """
    # Normalise to a per-molecule list so the loop is uniform regardless of input type.
    if isinstance(N_x1, int):
        N_list = [N_x1] * batch_size
    else:
        N_list = list(N_x1)
    ...
    x1_batch = torch.cat([
        torch.ones(N_list[i] + int(include_virtual_node), dtype=torch.long) * i for i in range(batch_size)
    ])
    ...
    for b in range(batch_size):
        ...
        # Build per-molecule bond graph (moved inside loop to support variable N_b).
        bond_adj = np.triu(1 - np.diag(np.ones(N_b, dtype=int)))
        ...
        x1_pos_T = torch.randn(N_b, 3) * prior_noise_scale
        if not scaffold_conditioning:
            x1_pos_T = x1_pos_T - torch.mean(x1_pos_T, dim=0)
        x1_x_T = torch.randn(N_b, num_atom_types) * prior_noise_scale
        x1_bond_edge_x_T = torch.randn(
            bond_edge_index.shape[1], len(params['dataset']['x1']['bond_types'])
        ) * prior_noise_scale

        x1_virtual_node_mask_ = torch.zeros(N_b + int(include_virtual_node), dtype=torch.long)
        ...
        num_nodes_counter += N_b + int(include_virtual_node)
...
def _initialize_x4_state(batch_size: int,
    ...
    ---------
    batch_size: The batch size.
    N_x4: The number of x4 nodes (int for uniform, list[int] of length batch_size for per-molecule counts).
    params: The parameters.
    prior_noise_scale: The prior noise scale.
    include_virtual_node: Whether to include a virtual node.
    ...
    if isinstance(N_x4, int):
        N_list = [N_x4] * batch_size
    else:
        N_list = list(N_x4)

    x4_batch = torch.cat([
        torch.ones(N_list[i] + int(include_virtual_node), dtype=torch.long) * i
    ...
    for b in range(batch_size):
        N_b = N_list[b]
        x4_pos_T = torch.randn(N_b, 3) * prior_noise_scale # t = T
        # NOT removing COM from x4
        x4_direction_T = torch.randn(N_b, 3) * prior_noise_scale # t = T
        x4_x_T = torch.randn(N_b, num_pharm_types) * prior_noise_scale

        x4_virtual_node_mask_ = torch.zeros(N_b + int(include_virtual_node), dtype = torch.long)
        if include_virtual_node:
